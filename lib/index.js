...
function rabbitmqAppender(config, layout) {
  ...
  const heartbeat = config.heartbeat || 0;
  ...
  amqplib.connect(con).then((c) => {
    ...
    debug('Connected. properties' + JSON.stringify(connection.serverProperties) );
    console.log(JSON.stringify(connection.serverProperties));
    publish();
  }).catch((e) => {
